...
import { View, Modal, Pressable, ScrollView, Alert, KeyboardAvoidingView, Platform } from "react-native";
...
import { useReviewReports, ReportReason, REPORT_REASONS } from "@/hooks/useReviewReports";
...
export const ReportReviewModal: React.FC<ReportReviewModalProps> = ({
  ...
  const handleNext = () => {
    if (!selectedReason) {
      Alert.alert("Select a Reason", "Please select a reason for reporting this review.");
      return;
    }
  setStep("details");
  };
  ...
  const ReasonOption: React.FC<{ 
    ...
    >
      <View className="flex-row items-center justify-between">
        ...
        {selected && (
          <View className={`w-5 h-5 rounded-full ml-3 ${
    ...
    >
      ...
      >
        <View className="flex-1 bg-black/50 justify-center px-4">
          <View className={`rounded-2xl max-h-[90%] ${
              ...
              {step === "reason" ? (
                <View className="p-6">
                  <P className="text-muted-foreground mb-6">
                    Help us understand what's wrong with this review. Your report is anonymous.
                  </P>
          ...
          {REPORT_REASONS.map((reason) => (
            ...
            onSelect={() => setSelectedReason(reason.value)}
                ...
                <View className="p-6">
                  ...
                  <Text className="font-medium mb-2">Additional Details (Optional)</Text>
