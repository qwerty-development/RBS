...
import { CreatePlaylistModal } from "./CreatePlaylistModal"; // now used in "inline" mode
...
export const AddToPlaylistModal: React.FC<AddToPlaylistModalProps> = ({
  ...
  const handleAddToPlaylists = useCallback(async () => {
    ...
    const successfulPlaylistIds = new Set<string>();
    ...
    try {
      for (const playlistId of selectedPlaylists) {
        ...
        if (success) {
          successCount++;
          successfulPlaylistIds.add(playlistId);
          const playlist = playlists.find((p) => p.id === playlistId);
          if (playlist && !addedPlaylistName) addedPlaylistName = playlist.name;
        }
      }
      ...
      if (successCount > 0) {
        ...
        for (const id of successfulPlaylistIds) newStates.set(id, true);
        setPlaylistStates(newStates);
  ...
  const handleCreateNewPlaylist = useCallback(
    async (data: { name: string; description: string; emoji: string }) => {
      ...
      if (newPlaylist) {
        ...
        setShowCreatePlaylist(false); // go back to list view
        await handleRefresh();
      }
    },
  ...
  useEffect(() => {
    if (!visible) {
      ...
      setSearchQuery("");
      setShowCreatePlaylist(false); // ensure we reset when modal closes
    }
  }, [visible]);
    ...
    >
      ...
      >
        <View className="flex-1 bg-gray-50 dark:bg-gray-900">
          ...
          {/* BODY: either the picker list OR the inline create view */}
          {showCreatePlaylist ? (
            // Inline create form (no nested RN Modal)
            <CreatePlaylistModal
              inline
              visible
