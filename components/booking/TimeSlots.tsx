...
import { 
  Clock, Wifi, CheckCircle, Sparkles, Users, 
  Crown, Car, TreePine, Utensils, Eye 
} from "lucide-react-native";
import * as Haptics from "expo-haptics";

import { Text } from "@/components/ui/text";
import { Button } from "@/components/ui/button";
import { TimeSlotBasic, SlotTableOptions, TableOption } from "@/lib/AvailabilityService";
...
export const TimeSlots = memo<{
  ...
  error?: string | null;
}>(({ slots, selectedTime, onTimeSelect, loading, showLiveIndicator = false, error }) => {
...
export const ExperienceSelector = memo<{
  ...
  time: string;
  partySize: number;
  onConfirm: (tableIds: string[], selectedTime: string, selectedDate: Date, partySize: number, selectedOption: any) => void;
  onBack: () => void;
  ...
  React.useEffect(() => {
    const loadOptions = async () => {
      setLoading(true);
      try {
        const availabilityService = (await import('@/lib/AvailabilityService')).AvailabilityService.getInstance();
  ...
  const handleConfirm = useCallback((tableIds: string[], selectedOption: TableOption) => {
