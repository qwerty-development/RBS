            ...
            >
              {availableOptions.map((time, index) => (
                <Pressable
                  ...
                  className={`p-3 ${
                    index < availableOptions.length - 1 ? 'border-b border-border/50' : ''
                  } ${time === value ? 'bg-primary/10' : 'bg-white'}`}
                  style={{ minHeight: 44 }}
                >
                  <Text className={`${time === value ? 'text-primary font-medium' : ''}`}>
                    {time}
                  </Text>
                </Pressable>
              ))}
...
const TableTypeSelector = React.memo<{
  ...
  const getTableTypeCount = useCallback((type: TableType) => {
  ...
  return (
    <View>
      <Text className="text-sm font-medium mb-3 text-muted-foreground">
        Table Types {selectedTypes.length > 0 ? `(${selectedTypes.length} selected)` : '(optional)'}
      </Text>
...
const PartySizeSelector = React.memo<{
  ...
  return (
    <View style={{ zIndex: expanded ? 9998 : 1 }}>
      <Text className="text-sm font-medium mb-2 text-muted-foreground">Party Size</Text>
            ...
            >
              {sizeOptions.map((size, index) => (
                <Pressable
                  ...
                  className={`p-3 ${
                    index < sizeOptions.length - 1 ? 'border-b border-border/50' : ''
                  } flex-row items-center gap-2 ${
                    size === selectedSize ? 'bg-primary/10' : 'bg-white'
                  }`}
                  style={{ minHeight: 44 }}
                >
                  <Users size={14} color={size === selectedSize ? '#3b82f6' : '#666'} />
                  <Text className={`${size === selectedSize ? 'text-primary font-medium' : ''}`}>
    ...
    >
      ...
      <View className="flex-row items-center gap-4 mb-3">
        ...
        {primaryOption && primaryOption.tableTypes.length > 0 && (
          <View className="flex-row items-center gap-1">
            <Text className="text-sm text-muted-foreground">
              {result.allTableTypes.map(type => TABLE_TYPES[type as TableType]?.icon || '🪑').join(' ')}
            </Text>
            <Text className="text-xs text-muted-foreground">
              {result.allTableTypes.map(type => TABLE_TYPES[type as TableType]?.label || type).join(', ')}
            </Text>
          </View>
        )}
      </View>
        ...
        <View className="bg-muted/30 rounded-lg p-2">
          ...
          {hasMultipleOptions && (
            <Text className="text-xs text-blue-600 dark:text-blue-400 mt-1">
              +{result.tableOptions.length - 1} more seating option{result.tableOptions.length > 2 ? 's' : ''}
            </Text>
          )}
        </View>
      )}
...
export const TimeRangeSelector: React.FC<TimeRangeSelectorProps> = ({
  ...
  const [allSearchResults, setAllSearchResults] = useState<TimeRangeResult[]>([]); // Store all results
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Search Error', 'Failed to search for available times. Please try again.');
  ...
  const handleSelectResult = useCallback((result: TimeRangeResult) => {
      ...
      <View className="flex-1 bg-background">
        ...
        <TouchableWithoutFeedback onPress={handleCloseAllDropdowns}>
          <ScrollView 
            ...
            style={{ zIndex: 1 }}
          >
            <View className="p-4 gap-6" style={{ zIndex: anyDropdownExpanded ? 1000 : 1 }}>
              ...
              <View style={{ zIndex: anyDropdownExpanded ? 5000 : 1 }}>
                <Text className="font-semibold text-lg mb-3">Select Time Range</Text>
            ...
            {hasSearched && (
              <View>
                  ...
                  <View className="flex-row items-center gap-2">
                    ...
                    <View className="bg-primary/10 rounded-full px-3 py-1">
                      <Text className="text-primary font-bold text-sm">
                        ...
                        {selectedTableTypes.length > 0 && allSearchResults.length !== filteredResults.length && (
                          <Text className="text-muted-foreground"> of {allSearchResults.length}</Text>
                        )}
                      </Text>
                    </View>
                  </View>
                ...
                {filteredResults.length === 0 ? (
                  ...
                  <View>
                    {filteredResults.map((result: TimeRangeResult, index: number) => (
                      <SearchResultCard
