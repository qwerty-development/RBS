...
export function UserRatingDashboard({
  ...
  const getRatingColor = () => {
    if (isExcellent) return "#22c55e"; // green
    if (isGood) return "#3b82f6"; // blue
    if (isRestricted) return "#f59e0b"; // amber
  ...
  const getRestrictionMessage = () => {
    ...
    if (isRestricted) {
      return "You can only make request bookings. Some restaurants may not accept your reservations.";
    }
    if (isGood) {
      return "You can make instant bookings at most restaurants.";
    }
    if (isExcellent) {
      return "You have excellent standing and can book anywhere instantly.";
    }
    return "";
  };
