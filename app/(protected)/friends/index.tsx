  ...
  const renderFriend = ({ item }: { item: Friend }) => (
    ...
    >
      <View className="flex-row items-center flex-1">
        <Image
          source={{ uri: item.avatar_url || `https://ui-avatars.com/api/?name=${item.full_name}` }}
    ...
    return (
      ...
      >
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1">
            <Image
              source={{ uri: otherUser?.avatar_url || `https://ui-avatars.com/api/?name=${otherUser?.full_name}` }}
              className="w-14 h-14 rounded-full bg-gray-100"
            />
            <View className="ml-3 flex-1">
              <Text className="font-semibold text-base">{otherUser?.full_name}</Text>
              <Muted className="text-sm">
        ...
        {isReceived && (
          <View className="flex-row mt-3 gap-2">
            ...
            <Button
              ...
              className="flex-1"
            >
              <X size={16} color={colorScheme === 'dark' ? 'white' : 'black'} className="mr-1" />
              <Text>Decline</Text>
            </Button>
          </View>
        )}
      ...
      <View className="flex-row bg-white dark:bg-gray-800 px-4 py-2 border-b border-gray-200 dark:border-gray-700">
        {[
          ...
          >
            <tab.icon 
              size={18} 
              color={activeTab === tab.id ? '#dc2626' : colorScheme === 'dark' ? '#9ca3af' : '#6b7280'} 
            />
            <Text 
              className={`ml-2 font-medium ${
                activeTab === tab.id ? 'text-red-600' : 'text-gray-600 dark:text-gray-400'
              }`}
            >
              {tab.label}
            </Text>
