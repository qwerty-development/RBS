...
export default function PlaylistInvitationsScreen() {
  ...
  const handleAcceptInvitation = useCallback(async (invitation: PlaylistInvitation) => {
  ...
  const handleRejectInvitation = useCallback(async (invitation: PlaylistInvitation) => {
    Alert.alert(
      'Reject Invitation',
  ...
  const renderInvitationItem = useCallback(({ item }: { item: PlaylistInvitation }) => {
    const permissionIcon = item.permission === 'edit' ? Edit3 : Eye;
    const permissionText = item.permission === 'edit' ? 'Can edit' : 'Can view';
        ...
        <View className="flex-row items-start justify-between mb-3">
          <View className="flex-1">
            <View className="flex-row items-center mb-1">
              ...
              <H3 className="flex-1" numberOfLines={1}>{item.playlist.name}</H3>
            </View>
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center">
                ...
                <Muted className="text-sm ml-1">
                  {item.playlist.item_count} restaurant{item.playlist.item_count !== 1 ? 's' : ''}
                </Muted>
              </View>
              
              <View className="flex-row items-center">
                {React.createElement(permissionIcon, { size: 14, color: "#6b7280" })}
      ...
      {invitations.length === 0 ? (
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Muted className="text-center mb-6">
            When someone invites you to collaborate on their playlist, you'll see the invitation here.
          </Muted>
