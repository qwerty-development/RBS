...
problematicSlots.forEach((slot, i) => {
  console.log(
    `${i + 1}. ${slot.time} (${slot.minutesFromNow} minutes from now)`,
  );
});
...
oldResults.forEach((slot, i) => {
  ...
  console.log(
    `${i + 1}. ${slot.time} (${slot.minutesFromNow} min) ${wouldCauseError ? "❌ WOULD CAUSE ERROR" : "✅"}`,
  );
});
...
if (problemSlots.length > 0) {
  ...
  problemSlots.forEach((slot) => {
    console.log(
      `   - ${slot.time} (only ${slot.minutesFromNow} minutes from now)`,
    );
  });
...
console.log("\n=== CONCLUSION ===");
console.log(
  "✅ The fix successfully prevents the display of time slots that are",
);
