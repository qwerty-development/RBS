...
const WHATSAPP_FUNCTION_URL = 'https://xsovqvbigdettnpeisjs.supabase.co/functions/v1/notify-restaurant-whatsapp';
 ...
 */
export async function notifyRestaurantWhatsApp(bookingId: string): Promise<WhatsAppNotificationResult> {
  try {
 ...
 * @param bookingId - The ID of the booking that was just created
 */
export async function notifyRestaurantWhatsAppNonBlocking(bookingId: string): Promise<void> {
  console.log('🔥 WhatsApp notification triggered (non-blocking) for booking:', bookingId);
  try {
    ...
    console.error('Unexpected error in non-blocking WhatsApp notification:', error);
  }
}
