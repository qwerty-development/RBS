...
function cleanAddressPart(part) {
  ...
  const cleaned = part.replace(/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/g, '').trim();
...
export async function getAddressFromCoordinates(latitude, longitude) {
  try {
    ...
    if (address) {
      ...
      const roadName = cleanAddressPart(address.road) || 
      ...
      const areaName = cleanAddressPart(address.neighbourhood) || 
      ...
      if (parts.length === 1) {
        const cityName = cleanAddressPart(address.city) || 
