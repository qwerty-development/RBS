...
export function useSocial() {
  ...
  const checkFriendship = useCallback(async (userId: string): Promise<boolean> => {
  ...
  // Like/Unlike a post
  const togglePostLike = useCallback(async (postId: string, isLiked: boolean) => {
  ...
  const getFriendSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const friendIds = friends?.reduce((acc: string[], f) => {
