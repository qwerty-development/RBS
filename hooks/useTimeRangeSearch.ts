...
export function useTimeRangeSearch() {
  ...
  const searchTimeRange = useCallback(
    async (params: TimeRangeSearchParams): Promise<TimeRangeResult[]> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        return results.map((result) => ({
          ...
          allTableTypes: result.tableOptions.flatMap(
            (option) => option.tableTypes,
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to search time range";
      ...
      return async (
        ...
        try {
          ...
          return results.map((result) => ({
            ...
            tables: result.tables,
            tableOptions: result.tableOptions,
            allTableTypes: Array.from(
              new Set(
                result.tableOptions.flatMap((option) => option.tableTypes),
              ),
            ) as any[], // Collect all unique table types
            totalCapacity: result.totalCapacity,
            requiresCombination: result.requiresCombination,
          }));
        } catch (err) {
