...
export function useRecommendations(context?: Partial<RecommendationContext>) {
  ...
  const buildUserProfile = useCallback(async (): Promise<UserProfile | null> => {
    ...
    try {
      ...
      reviews?.forEach((review) => {
        if (review.restaurant?.cuisine_type) {
          const ratings = cuisineRatings.get(review.restaurant.cuisine_type) || [];
      ...
      const priceRanges = bookings
        ?.map((b) => b.restaurant?.price_range)
        .filter((p): p is number => p !== undefined) || [];
      
      const priceRangePreference = priceRanges.length > 0
        ? Array.from(new Set(priceRanges))
        : [1, 2, 3];
      
      // 3.6 Analyze booking patterns
      const bookingTimes = bookings?.map((b) => new Date(b.booking_time)) || [];
  ...
  const calculateRecommendationScore = useCallback(
    (
      ...
      const cuisineScore = userProfile.cuisinePreferences.get(restaurant.cuisine_type) || 0;
      if (cuisineScore > 0) {
        const weight = 0.3;
      ...
      if (userProfile.dietaryRestrictions.length > 0) {
        const matchingOptions = restaurant.dietary_options?.filter((option) =>
        ...
        if (matchingOptions.length > 0) {
          const weight = 0.25;
          const matchScore = matchingOptions.length / userProfile.dietaryRestrictions.length;
      ...
      if (location) {
        ...
        if (distance <= userProfile.locationPreferences.maxDistance) {
          const weight = 0.15;
          const proximityScore = 1 - (distance / userProfile.locationPreferences.maxDistance);
      ...
      const restaurantHours = getRestaurantHoursForDay(restaurant, context.dayOfWeek);
      if (isOpenAtTime(restaurantHours, context.timeOfDay)) {
        ...
        if (context.timeOfDay === "breakfast" && restaurant.tags?.includes("brunch")) {
          totalScore += 0.05;
          reasons.push({
            type: "time_based",
            weight: weight + 0.05,
      ...
      if (context.weather) {
        if (context.weather === "rainy" && restaurant.ambiance_tags?.includes("cozy")) {
          totalScore += 0.05;
          reasons.push({
            type: "weather_based",
            weight: 0.05,
  ...
  const getRestaurantHoursForDay = (restaurant: Restaurant, dayOfWeek: number) => {
  ...
  const isOpenAtTime = (hours: { open: string; close: string }, timeOfDay: string): boolean => {
  ...
  }, [profile?.id, location, context, buildUserProfile, calculateRecommendationScore]);
