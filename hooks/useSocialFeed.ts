  ...
  const loadFeed = useCallback(
    async (limit: number = 20, offset: number = 0) => {
      ...
      try {
        ...
        const friendIds =
          friendsData?.reduce((acc: string[], f) => {
          ...
          setPosts((prev) =>
            prev.map((p) =>
              p.id === postId
                ? {
                    ...p,
                    _user_has_liked: false,
                    _likes_count: (p._likes_count || 1) - 1,
                  }
                : p,
            ),
          );
          ...
          setPosts((prev) =>
            prev.map((p) =>
              p.id === postId
                ? {
                    ...p,
                    _user_has_liked: true,
                    _likes_count: (p._likes_count || 0) + 1,
                  }
                : p,
            ),
          );
